...
http_res_t* http_perform(http_t* http) {
    ...
    if (res->response_code == BS_SUCCESS) {
        socket_block(socket);
        res->response_code = write_timeout(socket, http->time_out);
        if (res->response_code == BS_SUCCESS) {
            if (write(socket, http->req.mem, http->req.len) <= 0) {
                res->response_code = BS_SENDERR;
            } else {
                res->response_code = read_timeout(socket, http->time_out);
                if (res->response_code == BS_SUCCESS) {
        ...
        if(file == NULL){
            ...
            if (res->response_code == BS_SUCCESS) {
                if(write(socket, http->req.mem, http->req.len)<=0){
                    ...
                    res->response_code = read_timeout(socket, http->time_out);
                    if (res->response_code == BS_SUCCESS) {
                        if ((read_length = read(socket, buf, sizeof(buf))) > 0) {
    ...
    if (res->response_code == BS_SUCCESS) {
        ...
        if (res->response_code == BS_SUCCESS) {
            ...
            if (write(socket, http->req.mem, http->req.len)<=0){
                ...
                res->response_code = read_timeout(socket, http->time_out);
                if (res->response_code == BS_SUCCESS) {
                    while((read_length = (int)read(socket, buf, sizeof(buf))) > 0) {
                        data_append(&res->response, buf, (uint32_t)read_length);
